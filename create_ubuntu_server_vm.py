...
VM_RAM 		 = 4096  # Required 4 GB of RAM for eventual full-GUI Ubuntu
VM_CPUS 	 = 2
VM_DISK 	 = "ubuntu_auto_ysu.vdi"
DISK_SIZE_MB = 20000  # So roughly 20 GB
...
def create_vm():
	...
	# This will allocate the appropriate amount of RAM and disk
	# space for the VM, as well as specifying what OS type will
	...
	# install the minimal server setup that we need, with the 
	# passwordless user and the single htop package 
    run(f"VBoxManage storageattach {VM_NAME} \
			--storagectl 'IDE Controller' \
        	--port 1 \
        	--device 0 \
...
# After this, the next steps should be to: 
# 1) Add the necessary packages to the user-data "packages" section
# 2) Write a Bash script to install the GUI properly
# OR 
# 2) Write a shell script that can function from the login shell to 
#    burn a full Ubuntu version onto a USB 
